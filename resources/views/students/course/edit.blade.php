...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('user-course.update',$course->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            @if ($errors->any())
              ...
              <div class="mb-3 col-md-6">
                ...
                <select name="category" id="categorySelected" class="form-control">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" @if($course->course->category == $cat->id) selected @endif>{{ $cat->title }}</option>
                    @endforeach
                </select>
              </div>
              <div class="mb-3 col-md-6">
                ...
                <select name="sub_category" id="subCategory" class="form-control">
                    <option value="{{ $course->course->sub_category }}" selected>{{ $course->course->sub_cat->title }}</option>
                </select>
              </div>
              <div class="mb-3 col-md-6">
                <label class="form-label" for="exampleFormControlTextarea1">Courses</label>
                <select name="course" id="course" class="form-control">
                    <option value="{{ $course->course->id }}" selected>{{ $course->course->title }}</option>
                </select>
              </div>
            </div>
            <div class="mt-2">
